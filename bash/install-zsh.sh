...
CODE_CONNECT_BASE_URL="https://raw.githubusercontent.com/badaz/code-connect/main"
...
alias-exists () {
    ...
    if alias-exists "$name"; then
        log "Alias ${c_emph}$name${c_log} already registered in ${c_path}~/.zshrc${c_log}, skipping"
    else
        echo "alias $name='$val'" >> ~/.zshrc
        log "Adding alias ${c_emph}$name${c_log} to ${c_path}~/.zshrc"
    fi
}
...
# Add the aliases to ~/.zshrc if not already done
...
print ""
print "Restart your shell or reload your ${c_path}.zshrc${c_fg} to see the changes."
print ""
print "  ${c_emph}source ${c_path}.zshrc"
print ""
